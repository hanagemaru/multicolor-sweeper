...
import { getFirstClickExclusions, totalAdjacent } from "./game-core";
import type {
  Board,
  Coordinate,
  Evidence,
...
function colorBit(color: number): number {
  return 1 << (color + 1);
}

function mineMask(colorCount: number): number {
...
function coordinates(index: number): Coordinate {
...
function stateLabel(mask: number, colorCount: number): string {
...
function clueValues(board: Board, index: number, mode: SolverMode): number[] {
...
export function createVisibleState({
  ...
  return {
    colorCount,
    mineCount,
    domains: Array<number>(GRID_SIZE * GRID_SIZE).fill(allStates),
    clues: Array<number[] | null>(GRID_SIZE * GRID_SIZE).fill(null)
  };
}

function revealFromBoard(
  board: Board,
  ...
  const revealed: RevealedClue[] = [];
  const flatCells = board.cells.flat();
...
interface Constraint {
  ...
  cells: number[];
  target: number;
  kind: "local-color" | "local-total" | "global-total" | "subset-difference" | "global-difference";
  sourceClues: number[];
}
...
function makeConstraint(
  ...
  predicate: string,
  target: number,
  metadata: Pick<Constraint, "kind" | "sourceClues">
): Constraint {
...
function validateConstraint(constraint: Constraint): void {
...
function buildConstraints(visible: VisibleState): Constraint[] {
...
function cluesAreAdjacent(left: Constraint, right: Constraint): boolean {
...
function differenceConstraint(
  left: Constraint,
  right: Constraint,
  kind: "subset-difference" | "global-difference"
): Constraint | null {
...
function deriveSubsetConstraints(baseConstraints: Constraint[]): Constraint[] {
  const derived: Constraint[] = [];
  const seen = new Set<string>();
  const localsByClue = new Map<number, Map<string, Constraint>>();
  ...
  const append = (constraint: Constraint | null): void => {
  ...
  for (const [clue, predicates] of localsByClue) {
    ...
    if (global && localTotal) append(differenceConstraint(localTotal, global, "global-difference"));
  }
  return derived;
}
...
interface Deduction {
  ...
  predicate: string;
  allowedMask: number | null;
  rule: Constraint["kind"];
  evidence: Evidence;
}

function deductionFor(constraint: Constraint, index: number, action: Deduction["action"]): Deduction {
  const predicate = constraint.predicate;
  const color = predicate.startsWith("color:") ? Number(predicate.slice(6)) : null;
  let allowedMask: number | null;
  if (predicate === "mine") {
    allowedMask = action === "force" ? null : SAFE;
  } else {
    allowedMask = action === "force" ? colorBit(color as number) : null;
  }
  return {
    index,
    action,
...
function deductionsFromConstraint(constraint: Constraint): Deduction[] {
...
function applyDeduction(visible: VisibleState, deduction: Deduction): boolean {
...
export function reasonFromVisible(visible: VisibleState): {
  changes: TraceDeduction[];
  passes: number;
  ruleUsage: Record<string, number>;
} {
  const changes: TraceDeduction[] = [];
  ...
  while (true) {
    ...
    const pending = constraints.flatMap(deductionsFromConstraint);
    let changed = false;
...
function validateDomainsAgainstBoard(board: Board, visible: VisibleState): void {
  const flatCells = board.cells.flat();
  for (let index = 0; index < visible.domains.length; index += 1) {
    const truth = flatCells[index].mineColor;
...
function statsFromTrace(trace: TraceStep[]): SolverStats {
  ...
  for (const step of trace) {
    stats.revealedCells += step.revealed.length;
    ...
    stats.mineDeductions += step.deductions.filter(
      (entry) => entry.result !== "safe" && entry.result !== "unknown"
    ).length;
...
export function solveBoard(
  board: Board,
  { mode = "color", includeTrace = true }: { mode?: SolverMode; includeTrace?: boolean } = {}
): SolverResult {
  if (!board.generated || board.firstClick === null) {
    throw new Error("Board must be generated with a first click");
  }
  if (mode !== "color" && mode !== "mono") throw new Error("mode must be color or mono");
  ...
  let status: SolverResult["status"] = "stalled";
  let stallReason: SolverResult["stallReason"] = "no-logical-move";
...
export function firstClickSafetyIsVisible(board: Board): boolean {
  if (!board.firstClick) return false;
