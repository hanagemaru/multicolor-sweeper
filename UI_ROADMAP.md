...
完了範囲:
- 約140msの通常セル開封ポップ＋局所フラッシュ
- 起点から距離順に開封ポップ込み約300ms以内で広がる連鎖開封
- 13マス以上の大連鎖で開封完了付近に盤面全体の小さな追いポップ
- 爆弾セルの2段階ポップ・短いフラッシュ・衝撃リング・14粒子・3px以内の盤面シェイク
- 完成盤面を見せてから左上→右下へ進む強めのCLEAR波＋盤面全体の終端ポップ
- 連鎖距離と開封数5段階の両方を反映する開封音
- 設置/色変更と解除を区別する旗効果音（正誤による違いなし）
- Web Audio API生成の開封 / 連鎖 / 旗 / 爆発 / CLEAR効果音
- `prefers-reduced-motion`で動きと結果待ちを短縮
- 演出中も通常の盤面入力をブロックしない構造
...
比較検討中（EFFECT LAB）:
- 開封数1〜4はマス数と同数発音、5マス以上は短い上昇連打＋終止和音へ圧縮する音案
- 黄色系の光を保ったまま意図が伝わりやすくする、フレーム / 斜めスキャン / ピクセル十字 / 二重スクエア
- 現行案を残した爆発3種（PIXEL BURST / CINEMATIC BLAST / SHOCKWAVE）
- 盤面を主役にしたCLEAR 3種（CLEAR WAVE / VICTORY BOARD / SUPER CLEAR）
- `?effects-lab=1` で個別・系列連続・ランダム再生し、採用前に触感比較する
