...
- 言語切替: トップメニュー / 設定 / ランキングの `日本語 | EN`、選択保存、`UI_TERMINOLOGY.md` の表記基準
- PR #38で歯車・設定導線、見出し階層、共通UIトークン、固定サイズ方針を整理
...
- 上位10位を基本表示し、自分が10位圏外なら自分の前後3人も取得・表示
- 上位10位と自分周辺が重なる/隣接する場合は重複なく連結し、離れている場合だけ `…` を表示
...
完了範囲:
- `日本語 | EN` はトップメニューに残し、設定・ランキングにも配置
- トップメニュー右上にピクセル調の歯車ボタンを追加
- 設定画面でプレイヤー名を登録・変更
- hanage-hub側の共通プライバシーポリシー `https://hanage.app/privacy/` へ直接リンク
- 既存の `DELETE /api/player` を使い、確認付きのオンラインデータ削除を実装
- 削除成功後は匿名player ID、credential、保存済み表示名を消し、言語設定とローカル自己ベストは保持
- 320×480を含む小画面向けに設定・メニューの固定サイズと余白を整理
...
- 公開前設定・プライバシー導線: PR #34
- カスタムドメイン `mcsweeper.hanage.app`: PR #37前後で公開状態を反映
- 設定導線・UI共通トークン整理: PR #38
- 上位10位＋自分周辺ランキング表示: PR #39
- UIボタンの押下音: PR #42
- アイコンの描き直し（64グリッド）: PR #43
- プライバシーリンクの言語追従: PR #44
- BGM（トップ / プレイ中 / 結果）と音量調整: PR #46〜#49
- バックグラウンド復帰後の音の復旧: PR #51
