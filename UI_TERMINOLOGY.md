...
| オンラインデータ削除 | DELETE ONLINE DATA | オンラインデータを削除 |
| 削除確認 | DELETE ONLINE DATA? | オンラインデータを削除しますか？ |
| 削除実行 | DELETE | 削除 |
| 削除中 | DELETING... | 削除中… |
| 削除完了 | DATA DELETED | 削除しました |
| 削除失敗 | DELETE FAILED | 削除できませんでした |
...
- 言語切替はトップメニューとランキング画面に `日本語 | EN` として横一列で表示し、歯車内の設定画面やプレイHUDには置かない。
...
- トップメニューから開いたランキングは `BACK` / `戻る` のみ、結果から開いたランキングは `RESULT` と `MENU` を表示する。
- 歯車ボタンのアクセシブルネームは英語 `SETTINGS`、日本語 `設定` とする。
- オンラインデータ削除の確認文は、名前とランキング記録が削除され、取り消せないことを明記する。
