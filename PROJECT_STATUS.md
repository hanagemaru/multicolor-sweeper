...
- タップ開封、3色4方向/4色5方向の色旗・無色旗、旗総数判定Chord、勝敗を最小UIで実装済み
...
- ユーザー向け用語を `BOMBS` / `FLAGS` に統一し、HUDを `TIME` / `FLAGS` に整理。旗数超過は許可し、`FLAGS` を負数の警告表示にする（PR #11、main反映済み）
...
- 日本語・英語の翻訳定義と、画面文言・ARIAラベル・操作ガイド・生成エラーを一括して切り替える基盤を実装（P1-C、PR #15、main反映済み）
- 端末言語の検出・選択内容の保存・HTML `lang` 追従も基盤として実装済み。現在のUIからは呼び出さず、将来の再有効化用に保持する
- P1-Cの利用方針を見直し、言語切替UIは一旦非表示。通常UIは英語を基本とし、初手待ち・プレイ中の操作案内だけ英日併記に整理（フォローアップPR作業中）
- プレイ中の `MULTICOLOR SWEEPER` / `TIME ATTACK` を外し、開始画面だけにタイトルを表示する構成へ変更（フォローアップPR作業中）
...
- 初手待ち・生成中・プレイ中だけスクロールを固定し、設定・結果画面は従来どおりスクロール可能（P1-B着手前のmain時点）
- 低い画面ではプレイ中の余白・間隔を圧縮
- Seed / attempt / generation ms の開発表示をプレイ画面から削除し、初手開封後のレイアウト変化を解消
- 生成中も操作ガイドを保持し、初手待ち → 生成中 → プレイ中の盤面サイズ変化を解消
- 難易度選択の `15 BOMBS` / `20 BOMBS` / `25 BOMBS` を維持し、ユーザー向け `MINES` 表示を廃止
...
- `CONDITION C` / `NO-GUESS` を通常UIから削除し、生成中・生成エラーを一般ユーザー向けの文言へ整理
- HTML / PWA metadata の説明文から内部生成方式を外し、「3色・4色で遊ぶマインスイーパー系パズルゲーム」に整理
- プレイ中の開始画面への戻り導線を `MENU` に統一
- 開始画面を含む全フェーズでページ全体を100dvhに固定し、結果表示による縦スクロールと盤面移動を防止
- CLEAR / GAME OVER / 生成エラーを盤面上のオーバーレイへ移し、`RETRY` / `MENU` と答え合わせ導線を整理
- CLEARだけクリアタイムを表示し、GAME OVERでは敗北時タイムを表示しない
- 「盤面を見る」で閉じた結果を、盤面上の小さな `RESULT` ボタンから再表示可能にした
...
- P0時点のヘッドレスChromium確認では320×480 / 320×568 / 390×844で実プレイ確認
  ...
  - P1-B着手前は結果画面でスクロール固定が解除されることを確認
- P1-B差分は `npm run typecheck` / `npm test` / `npm run build` 成功
- 3色/4色の方向判定、3色の未使用右下方向、色数外の不正旗拒否は自動テストで確認
- PR #8 / #9 / #11 / #13 / #14 / #15 のGitHub Actions CIで typecheck / test / build 成功
- P1-C差分は `npm run typecheck` / `npm test`（43件）/ `npm run build` 成功

## 確定事項

- 製品版の採用フィルタは条件C
- 条件C / No-Guess は生成・検証用の内部仕様として保持し、通常のユーザーUIには表示しない
- Chordは旗総数判定（旗総数がClue合計と一致すれば色の内訳を問わず実行。誤配置時は爆発し得る）
- 生成中表示の開始閾値は120ms
- タイマーはWorker生成完了・初手開封後に開始
- 開発用のSeed / attempt / 生成時間は通常UIには表示しない
- 開始画面を含む全フェーズでページ全体を100dvhに固定する。端末高さが極端に低い場合だけ、開始パネル内のスクロールへフォールバックする
- 初手待ち・生成中・プレイ中・結果表示は同じレイアウト高さを維持する
...
- 現段階では言語切替UIを表示せず、通常UIは英語を基本とする。初手待ちとプレイ中の操作案内だけ英日併記する
- 翻訳定義・言語検出・保存処理・テストは保持し、ランキングなど複雑な文言が増えた段階で再有効化を検討する
- HTML / PWAの静的metadataと現在の実行中HTML `lang` は英語を基準とする
...
1. P1-D 旗操作・爆発・クリアの視覚フィードバック
2. P1-E 効果音とミュート
3. P1-F 端末・アクセシビリティ最終QA
4. 6部門ランキングのバックエンド選定とデータ設計
